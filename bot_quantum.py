#!/usr/bin/env python3
"""
⚛️ QUANTUM BOT PRO - SINAIS TELEGRAM (14 ESTRATÉGIAS) + FILTROS DE QUALIDADE
🕯️ 12 Quadrantes + 5-2-0 + Chinesa 3.0
🛡️ Filtros: EMA9, Tendência (ADX), Pavio, Vela Forte
...
# ═══════════════════════════════════════════════════════════
CONFIANCA_MINIMA = 65        # Aumente para 70 para menos sinais
USAR_FILTRO_PAVIO = True     # Rejeita pavios longos
USAR_FILTRO_VELA_FORTE = True # Evita volatilidade extrema
USAR_FILTRO_TENDENCIA = True # Só opera a favor da tendência
USAR_FILTRO_EMA9 = True      # Novo filtro de EMA9
USAR_FILTRO_HORARIO = True   # Evita sessão asiática (baixa liquidez)
INTERVALO_MINIMO = 300       # Segundos entre sinais (5 min)

def banner():
    print("⚛️ QUANTUM BOT PRO - Sinais Telegram | 14 Estratégias | Filtros de Qualidade")
...
class Filtros:
    ...
    @staticmethod
    def sinal_alinhado_ema(velas, direcao):
        """CALL: fechamento acima da EMA9; PUT: fechamento abaixo da EMA9"""
        ema = Filtros.ema9(velas)
        if ema is None:
            return False
    ...
    def horario_ok():
        ...
        hora = agora.hour
        # Evita sessão asiática (22h-5h) e madrugada
    ...
    def buscar_sinal(self):
        ...
        for nome_par, velas in self.velas.items():
            ...
            for s in sinais_quad:
                ...
                # Aplica filtros
                if USAR_FILTRO_PAVIO and not Filtros.pavio_ok(velas, d):
                    continue
                if USAR_FILTRO_VELA_FORTE and not Filtros.vela_forte_ok(velas):
                    continue
                if USAR_FILTRO_EMA9 and not Filtros.sinal_alinhado_ema(velas, d):
                    continue
                # Se passou, retorna
                return {'ativo': nome_par, 'direcao': d, 'confianca': conf, 'estrategia': s['nome'], 'velas': velas}
            # 5-2-0
...
🎯 Assertividade: {tx}%"""
    ...
    async def executar(self):
        banner()
        print("⚛️ Bot Pro Telegram com filtros de qualidade iniciando...")
        self.tg.send("🔥 *QUANTUM BOT PRO ATIVADO*\n📊 14 Estratégias | M1\n🛡️ Filtros: EMA9, Pavio, Vela Forte, Horário\n⏱️ Sinais a cada 5min | Placar + Catalogador")
        while True:
            try:
                await self.atualizar_velas()
