...
║   👁️ Visão Gráfica | 📊 Volume | 🎯 Score Dinâmico ║
║   🔗 Correlação | 🛡️ Zona Segura | 😴 Descanso     ║
╚══════════════════════════════════════════════╝
"""
...
def banner():
    ...
    print(f"║   📊 Vol | 🎯 Score Dinâmico | 🔗 Corr    ║")
    print(f"║   🛡️ Zona Segura | 😴 Descanso           ║")
    print(f"╚══════════════════════════════════════════════╝{C.E}")
...
class Telegram:
    ...
    def send(self,txt):
        try:requests.post(f"{self.u}/sendMessage",json={"chat_id":self.c,"text":txt,"parse_mode":"Markdown"},timeout=3)
        except:pass
...
class Fortaleza:
    ...
    def analisar(self,v):
        try:
            ...
            bs=m+2*s;bi=m-2*s
            h_list=[x['high'] for x in v[-5:]];l_list=[x['low'] for x in v[-5:]]
            h=max(h_list);l=min(l_list)
...
class CerebroVisual:
    def __init__(self):
        ...
        self.score_minimo=60  # Dinâmico: começa em 60
        self.score_base=60
        ...
        self.sinais_operados=0  # Controle para score dinâmico
        self.ultimo_loss_time=0
        self.em_descanso=False
        self.descanso_ate=0

    def atualizar_score_dinamico(self):
        """Ajusta o score mínimo conforme desempenho recente"""
        if self.losses_seguidos>=3:
            self.score_minimo=min(self.score_base+15,75)
        elif self.losses_seguidos>=2:
            self.score_minimo=min(self.score_base+10,70)
        elif self.wins_seguidos>=5:
            self.score_minimo=self.score_base  # Volta ao normal
        elif self.wins_seguidos>=3:
            self.score_minimo=min(self.score_base+5,65)
        else:
            self.score_minimo=self.score_base

    def verificar_descanso(self):
        """Retorna (True, msg) se estiver em período de descanso"""
        if self.em_descanso:
    ...
    def registrar_loss(self):
        ...
        if self.losses_seguidos>=3:
            ...
            self.descanso_ate=time.time()+1800  # 30 minutos
        elif self.losses_seguidos>=2:
            self.em_descanso=True
            self.descanso_ate=time.time()+600   # 10 minutos
    ...
    def filtro_volume(self,velas):
        try:
            ...
            if vol_medio>0 and vol_atual<vol_medio*1.2:
                return False,f"📊 Volume baixo ({vol_atual:.0f} < {vol_medio*1.2:.0f})"
            return True,""
        except:return True,""

    # Filtro de Correlação (Melhoria #6) - Aplicado no Bot
    def filtrar_correlacao(self, sinais_por_par):
        """Retorna pares que devem ser bloqueados por conflito de correlação"""
        bloqueados=set()
        # EURUSD e GBPUSD têm alta correlação (>80%)
        if 'EURUSD' in sinais_por_par and 'GBPUSD' in sinais_por_par:
            ...
            if sinal_eur['direcao']!=sinal_gbp['direcao']:
                # Conflito: um CALL outro PUT -> Bloquear ambos
                bloqueados.add('EURUSD')
                bloqueados.add('GBPUSD')
        return bloqueados

    # Filtro de Zona de Segurança (Melhoria #7)
    def filtro_zona_seguranca(self,velas):
        try:
            ...
            if dist_resistencia<0.0005 or dist_suporte<0.0005:
    ...
    def avaliar(self,sinal,velas,sinais_por_par=None):
        ...
        if nota_visual<50:return False,0,[f"👁️ Gráfico ruim ({detalhes_visual})"],nota_visual,detalhes_visual
        # Score
        score=0;motivos=[]
    ...
    def conectar(self):
        for tentativa in range(5):
            try:
                ...
                if ok:self.ok=True;self.erros=0;print(f"  {C.G}✅ Reconectado!{C.E}");return True
                print(f"  {C.Y}⚠️ Tentativa {tentativa+1}/5{C.E}");time.sleep(5*(tentativa+1))
            except Exception as e:print(f"  {C.Y}⚠️ Erro reconexão: {str(e)[:30]}{C.E}");time.sleep(5*(tentativa+1))
                ...
                if c and len(c)>0:
                    ...
                    for x in c[-qtd:]:
                        if isinstance(x,dict):
                            try:self.velas[nome].append({'time':datetime.fromtimestamp(x.get('from',0)),'open':float(x['open']),'high':float(x['max']),'low':float(x['min']),'close':float(x['close']),'volume':int(x.get('volume',0))})
                            except:pass
                    return len(c)
...
class Bot:
    def __init__(self):
        ...
        # Para filtro de correlação: armazena o último sinal por par
        self.ultimos_sinais={}
...
📊 Placar: 🟢{self.p.w}W 🟡{self.p.g1}G1 🔴{self.p.l}L"""
    ...
    async def esperar(self,seg=60):
        try:
            ...
            alvo+=timedelta(seconds=seg);e=max(0,(alvo-agora).total_seconds())
            if e>0:await asyncio.sleep(e);self.iq.atualizar()
        except:pass
    ...
    async def catalogacao_inicial(self):
        ...
        for nome in ATIVOS_OTC:
            ...
            for i in range(35,len(velas)-1):
                janela=list(velas)[i-35:i+1];d,cf,num=self.m.analisar_completo(janela)
                if d:
            ...
            else:print(f"  {C.Y}⚠️ {nome}: Nenhum sinal encontrado nas velas{C.E}")
        print(f"  {C.GOLD}{'─'*50}{C.E}")

    async def run(self):
        banner()
        print(f"\n  ⚛️ Iniciando Quantum IA com 5 Melhorias...\n")
        if not self.iq.conectar():print(f"  ❌ Falha conexão!");return
        self.iq.atualizar();await self.catalogacao_inicial()
        print(f"\n  ✅ QUANTUM IA INICIADA | 🧠 Cérebro Visual | 🔄 1 GALE | 📊📈🛡️😴\n")
        self.tg.send(f"⚛️ *QUANTUM IA + 5 MELHORIAS*\n📊 Volume | 🎯 Score Dinâmico\n🔗 Correlação | 🛡️ Zona Segura | 😴 Descanso\n⏰ {datetime.now().strftime('%H:%M:%S')}")
        while True:
            try:
                agora=datetime.now()
                ...
                if not self.op:
                    try:
                        ...
                        # Coletar sinais disponíveis para correlação
                        sinais_por_par={}
                        for nome in ATIVOS_OTC:
                        ...
                        if sinal and time.time()-self.ult>25:
                            ...
                            if aprovado:
                                ...
                                print(f"\n⚛️ #{self.sinais} {sinal['ativo']}-OTC {sinal['direcao']} | {sinal['confianca']:.0f}% | 🧠{sinal.get('estrategias',0)}/5 | 🛡️{score:.0f}/{self.cerebro.score_minimo} | 👁️{nota_visual:.0f}/100 ({detalhes_visual}) | ⏰ {he}")
